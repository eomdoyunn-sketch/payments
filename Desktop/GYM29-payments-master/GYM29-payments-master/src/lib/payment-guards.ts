...
export function checkRequiredAgreements(
  ...
  // 안전하게 companyAgreements 처리
  const companyAgreements = settings.companyAgreements || { default: {}, companies: {} }
  const companySpecificAgreements = companyAgreements.companies?.[companyId]
  const defaultAgreements = companyAgreements.default || {}
  
  const agreementConfig = companySpecificAgreements || defaultAgreements
  
  const requiredAgreements = Object.entries(agreementConfig)
    .filter(([_, config]) => config && config.enabled && config.required)
    .map(([type]) => type)
  
  const allRequiredAgreed = requiredAgreements.every(type => agreementChecks[type])
  
  if (!allRequiredAgreed) {
    return {
      canPass: false,
      reason: "필수 동의를 체크하면 결제 버튼이 활성화됩니다.",
      type: "warning"
    }
  }
