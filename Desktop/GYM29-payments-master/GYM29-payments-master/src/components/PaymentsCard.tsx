...
export function PaymentsCard({
  ...
  // 전역 설정 사용 (전역 설정이 있으면 우선 사용, 없으면 로컬 설정 사용)
  const { settings: localSettings } = useSettings()
  const rawSettings = globalSettings || localSettings
  
  // 전역 설정과 로컬 설정의 구조 차이를 맞춰주기
  const settings = React.useMemo(() => {
    if (!rawSettings) return localSettings
    
    // 전역 설정이 있으면 로컬 설정과 병합하여 호환성 확보
    return {
      ...localSettings,
      ...rawSettings,
      // companyAgreements 구조 맞춰주기 (global -> default 변환)
      companyAgreements: (() => {
        const rawAgreements = rawSettings.companyAgreements
        if (!rawAgreements) return localSettings.companyAgreements || {
          default: {
        ...
        // global을 default로 변환
        return {
          default: rawAgreements.global || rawAgreements.default || {
            personal: { enabled: true, required: true },
            sensitive: { enabled: true, required: true },
            utilization: { enabled: false, required: false }
          },
          companies: rawAgreements.companies || {}
        }
      })()
    }
  ...
  React.useEffect(() => {
    console.log('💳 PaymentsCard 설정:', {
      ...
      localPrices: localSettings?.membershipPrices,
      finalPrices: settings?.membershipPrices
    })
  }, [globalSettings, localSettings, settings])
  ...
  const [agreementChecks, setAgreementChecks] = React.useState<Record<string, boolean>>({
    personal: false,
    sensitive: false,
    utilization: false
  })
  ...
  }, [settings, user.companyId, company, selectedProducts, selectedLocker, agreementChecks, verificationStatus, products])
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // 사용자에게 카드 정보 선택 안내
      toast.info('카드사와 할부 기간을 선택한 후 결제를 진행해주세요.')
      ...
      if (error?.message?.includes('카드 결제 정보를 선택해주세요') || error?.code === 'NEED_CARD_PAYMENT_DETAIL') {
        toast.error('카드사와 할부 기간을 선택해주세요.', {
          description: '결제 수단에서 카드사를 선택하고 할부 기간을 설정한 후 다시 시도해주세요.'
        })
        ...
        toast.error('결제 시스템에 일시적인 문제가 발생했습니다. 브라우저를 새로고침하거나 잠시 후 다시 시도해주세요.')
      } else {
  ...
  return (
    <Card className="w-full max-w-2xl">
      ...
      <CardContent className="space-y-6">
        ...
        {/* 동의 영역 */}
        <div className="space-y-4">
          <Label className="text-base font-medium">동의서</Label>
          <div className="space-y-3">
            {agreements.map((agreement) => {
              // 전역 설정에서 동의서 설정 가져오기 (계열사별 설정 우선, 없으면 기본 설정)
              const companyAgreement = settings.companyAgreements.companies[company.id]?.[agreement.type] || 
                                     settings.companyAgreements.default[agreement.type]
              const isEnabled = companyAgreement.enabled
              const isRequired = companyAgreement.required
              
              // 비활성화된 동의서는 표시하지 않음
              if (!isEnabled) return null
              
              return (
                <div key={agreement.type} className="flex items-center justify-between p-3 border rounded-lg">
                  <div className="flex items-center gap-3">
                    <FileText className="h-4 w-4 text-muted-foreground" />
                    <div>
                      <span className="text-sm font-medium">
                        {agreement.title} {isRequired ? "(필수)" : "(선택)"}
                      </span>
                      <div className="flex items-center gap-2 mt-1">
                        <Badge variant={isRequired ? "default" : "secondary"} className="text-xs">
                          {isRequired ? "필수" : "선택"}
                        </Badge>
                        <Badge variant="outline" className="text-xs">{agreement.version}</Badge>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      onClick={() => {
                        ...
                        setAgreementViewed(prev => ({ ...prev, [agreement.type]: true }))
                      }}
                    >
                      전문보기
                    </Button>
                    <Checkbox
                      id={agreement.type}
                      checked={agreementChecks[agreement.type]}
                      disabled={!agreementViewed[agreement.type]}
                      onCheckedChange={(checked) => 
                        setAgreementChecks(prev => ({ ...prev, [agreement.type]: checked as boolean }))
                      }
                    />
                  </div>
                </div>
      ...
      {selectedAgreement && (
        <AgreementModal
          ...
          initialAgreed={agreementChecks[selectedAgreement.type]}
        />
      )}
    </Card>
  )
