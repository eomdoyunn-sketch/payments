...
export function PaymentDataTable({
  ...
  const getUsagePeriod = (payment: PaymentData) => {
    ...
    const membershipPeriod = payment.membershipPeriod || 3 // 기본값 3개월
    const membershipEnd = calculateEndDate(membershipStart, membershipPeriod)
    
    const lockerStart = settings.lockerStartDate || '2025-01-01'
    const lockerPeriod = payment.lockerPeriod || 0
    const lockerEnd = calculateEndDate(lockerStart, lockerPeriod)
