...
// 화이트리스트 상태 확인 (업로드 여부 및 개수)
export async function getWhitelistStatus(
  companyCode: string,
  productType: 'fullDay' | 'morning' | 'evening'
): Promise<{ hasWhitelist: boolean; count: number; isOverLimit: boolean; allocatedQuota: number }> {
  try {
    const supabase = await createClient()
    ...
    if (error) {
      ...
      return { hasWhitelist: false, count: 0, isOverLimit: false, allocatedQuota: 0 }
    }

    // 배분 수량 조회 (companies 테이블에서)
    ...
    if (companyError) {
      console.error('Error fetching company quota:', companyError)
      return { hasWhitelist: false, count: 0, isOverLimit: false, allocatedQuota: 0 }
    }

    const allocatedQuota = companyData?.quota?.[productType] || 0
    const whitelistCount = count || 0
    const isOverLimit = whitelistCount > allocatedQuota
    ...
    console.error('getWhitelistStatus error:', error)
    return { hasWhitelist: false, count: 0, isOverLimit: false, allocatedQuota: 0 }
  }
}
