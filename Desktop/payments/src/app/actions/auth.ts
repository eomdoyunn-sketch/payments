...
export async function signup(formData: FormData): Promise<AuthResult> {
  try {
    ...
    if (!privacyAgreed) {
      return { error: '개인정보 수집·이용 동의서에 동의해주세요.' }
    }
    ...
    const agreementInserts = [
      {
        ...
        agreed: termsAgreed,
        agreed_at: termsAgreed ? new Date().toISOString() : null,
        ip_address: null, // 필요시 request에서 추출
        user_agent: null  // 필요시 request에서 추출
      },
      {
        user_id: authData.user.id,
        agreement_type: 'privacy',
        agreed: privacyAgreed,
        agreed_at: privacyAgreed ? new Date().toISOString() : null,
        ip_address: null,
        user_agent: null
      }
    ]

    // 마케팅 동의는 회원가입 시 비활성화되므로 저장하지 않음
    // marketingAgreed가 true인 경우에만 저장
    if (marketingAgreed) {
    ...
    if (agreementError) {
      console.error('동의서 저장 오류:', agreementError)
      // 동의서 저장 실패해도 회원가입은 성공으로 처리
    }
